...
def main():
    ...
    print('The average mass of 30 meteors:\n', compute_average_mass(ml_data['meteorite_landings'], 'mass (g)'))

    hemisphere_dict = {'Northern & Eastern': 0,
                     'Northern & Western': 0,
                     'Southern & Eastern': 0,
                     'Southern & Western': 0 }

    for row in ml_data['meteorite_landings']:
        hemisphere_dict[check_hemisphere(float(row['reclat']), float(row['reclong']))] += 1

    print("\n Hemisphere summary data:")
    for key, value in hemisphere_dict.items():
        print('There were ', value, ' meteors found in the ', key, 'quadrant')
